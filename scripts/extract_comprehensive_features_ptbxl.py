"""
Comprehensive ECG Feature Extraction for PTB-XL Dataset
========================================================

Extracts ALL features from the academic tables:
- Statistical Features (Table V)
- Temporal Features (Table VI)  
- Morphological Features (Table VII)
- Spectral Features (Table VIII)
- Time-Frequency Features (Table IX)
- Nonlinear Features (Table X)

This script processes the entire PTB-XL dataset and extracts comprehensive features.
"""
...
# Optimization: reduce verbosity for speed
VERBOSE_PER_LEAD = False  # Set to False for faster processing (no per-lead print)
SKIP_ENTROPY_FEATURES = True  # Skip ApEn/SampEn for 10x speed boost (still compute other nonlinear)
...
def extract_statistical_features(signal_segment: np.ndarray) -> Dict[str, float]:
    ...
    Returns: mean, variance, std, median, min, max, range, skewness, kurtosis,
             IQR, RMS, CV, MAD, peak-to-peak, energy, signal_power
    """
    features = {}
...
def find_p_qrs_t_waves(ecg_beat: np.ndarray, r_peak_idx: int, fs: int) -> Dict[str, int]:
    ...
    Returns indices relative to the beat segment.
    """
    indices = {}
    n = len(ecg_beat)
    
    # Q: local minimum before R (search 80ms before R)
    ...
    # S: local minimum after R (search 80ms after R)
    ...
    # P wave: search backwards from Q (typically 120-200ms before QRS)
    ...
    # T wave: search forward from S (typically 100-300ms after QRS)
...
def extract_morphological_features(ecg_signal: np.ndarray, fs: int, r_peaks: np.ndarray = None) -> Dict[str, float]:
    ...
    for i in range(1, len(r_peaks) - 1):
        try:
            # Extract beat segment (from previous R to next R)
            ...
            p_dur = int(0.08 * fs) / fs * 1000  # Approximate
            p_durs.append(p_dur)
            
            t_dur = int(0.12 * fs) / fs * 1000  # Approximate
            t_durs.append(t_dur)
...
def extract_spectral_features(ecg_signal: np.ndarray, fs: int) -> Dict[str, float]:
    ...
    try:
        ...
        # Spectral Flux (change between frames - use FFT on windows)
        ...
        # Return NaN for all features if extraction fails
...
def extract_time_frequency_features(ecg_signal: np.ndarray, fs: int) -> Dict[str, float]:
    ...
    try:
        ...
        # Wavelet Transform (CWT) - using complex Morlet
...
def approximate_entropy(signal_data: np.ndarray, m: int = 2, r: float = None) -> float:
    """Calculate Approximate Entropy (ApEn) - optimized with downsampling"""
    try:
        # Downsample to 200 samples for faster computation (entropy doesn't need high freq)
        if len(signal_data) > 200:
            step = max(1, len(signal_data) // 200)
...
def sample_entropy(signal_data: np.ndarray, m: int = 2, r: float = None) -> float:
    """Calculate Sample Entropy (SampEn) - optimized with downsampling"""
    try:
        # Downsample to 200 samples for faster computation
        if len(signal_data) > 200:
            step = max(1, len(signal_data) // 200)
            signal_data = signal_data[::step]
...
def detrended_fluctuation_analysis(signal_data: np.ndarray) -> float:
    """Calculate DFA (Detrended Fluctuation Analysis) alpha exponent - optimized"""
    try:
        # Downsample to 500 samples for faster computation
...
def extract_nonlinear_features(ecg_signal: np.ndarray, fs: int, r_peaks: np.ndarray = None) -> Dict[str, float]:
    ...
    # Skip expensive entropy calculations if flag is set (10x speedup)
    if not SKIP_ENTROPY_FEATURES:
        # Approximate Entropy
    ...
    # Poincaré Plot Indices (need RR intervals)
    if r_peaks is None or len(r_peaks) == 0:
        r_peaks = detect_r_peaks(ecg_signal, fs)
...
def extract_all_features_from_signal(ecg_signal: np.ndarray, fs: int, lead_name: str = "") -> Dict[str, Any]:
    """
    Extract ALL features from a single ECG lead signal.
    """
    all_features = {'lead_name': lead_name, 'sampling_freq': fs}
    
    # Detect R-peaks once (used by multiple feature extractors)
    r_peaks = detect_r_peaks(ecg_signal, fs)
    
    # Extract each category of features (silent mode for speed)
...
def process_record(record_path: str, record_id: int, fs: int = SAMPLING_FREQ) -> Dict[str, Any]:
    """
...
def main():
    ...
    print("=" * 80)
    print("PTB-XL COMPREHENSIVE FEATURE EXTRACTION")
    print("=" * 80)
    print()
    ...
    # Filter for specific sampling rate
    ...
    for batch_start in range(0, len(df), BATCH_SIZE):
        ...
        for idx, row in tqdm(batch_df.iterrows(), total=len(batch_df), desc="Batch progress"):
            record_id = row['ecg_id']
            filename = row['filename']
            record_path = os.path.join(PTBXL_ROOT, filename)
            
            result = process_record(record_path, record_id, SAMPLING_FREQ)
    ...
    for result in all_results:
        if result['success']:
            ...
            # Flatten features from all leads
